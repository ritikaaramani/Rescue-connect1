...
from .feature_builder import build_model2_features
...
def call_model3(
    ...
    if use_mock:
        ...
        dyn_root = _PROJECT_ROOT / "dynamic-rerouting"
...
def run_emergency_pipeline(
    ...
    logger.info(f"Starting emergency routing pipeline: {start} -> {destination} in {city_name}")
    ...
    logger.info(f"Model1 prediction completed: congestion_t10={model1_output['congestion_t10']:.3f}")
    ...
    logger.info(f"Route generation and Model2 scoring completed: {len(scored_routes)} routes")
    ...
    logger.info(f"Model3 rerouting decision: {model3_decision['action']}")
    ...
    result = {
        ...
        "model1_prediction": model1_output,
...
@app.post("/best-route")
async def best_route(request: BestRouteRequest):
...
def debug_routes():
    # For demo purposes, run a mock pipeline and return routes
    try:
        ...
        logger.error(f"Debug routes failed: {str(e)}")
