...
enum _Phase { idle, searching, routing, results }
...
class _HomeScreenState extends ConsumerState<HomeScreen>
  ...
  final _unifiedService = UnifiedService();
  ...
  Future<void> _findNearestHospitals() async {
    final pos = _originCoord ?? LatLng(_city.lat, _city.lng);
    _snack('Querying AI Brain for nearest available ICU beds...', error: false);
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            Text('NEAREST AVAILABLE HOSPITALS',
                style: GoogleFonts.rajdhani(color: kAiCyan, fontSize: 13, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
            ...
            ...hospitals.map((h) => Container(
              ...
              child: Row(
                children: [
                  const Icon(Icons.local_hospital, color: kDanger, size: 20),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(h.name, style: GoogleFonts.rajdhani(color: Colors.white, fontWeight: FontWeight.bold)),
                        Text('${h.distanceKm} km away · ${h.icuBeds} ICU beds available', 
                  ...
                  TextButton(
                    onPressed: () {
                      setState(() {
                        _destCtrl.text = h.name;
                        _destCoord = h.location;
                      });
                      Navigator.pop(ctx);
                      _analyzeRoute();
                    },
                    child: Text('ROUTE', style: GoogleFonts.rajdhani(color: kAiCyan, fontWeight: FontWeight.bold)),
                  ),
                ],
              ),
            )),
  ...
  Future<void> _reportIncident() async {
    ...
    final success = await _unifiedService.reportIncident(pos, 'Accident', 8);
    if (success) {
      _snack('Accident reported. RL Agent notified for real-time rerouting.', error: false);
      HapticFeedback.mediumImpact();
    } else {
      _snack('Failed to sync incident with central dispatch.', error: true);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final result = _result;
    final activeDispatches = ref.watch(activeDispatchesProvider);
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        // ── MAP ──────────────────────────────────────────────────────────────
        _buildMap(result, activeDispatches),
  ...
  Widget _buildMap(RouteResult? result, List<ActiveDispatch> activeDispatches) {
  ...
  List<Widget> _buildIdleContent() => [
        ...
        // Deployment Worthy Extras (Phase 4)
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 20),
          child: Row(
            children: [
              ...
              child: Stack(
                ...
                children: [
                  Container(
                    ...
                    child: Center(
                      child: Icon(Icons.videocam_off, color: Colors.white.withOpacity(0.1), size: 40),
                    ),
                  ),
                  Center(
                    child: Row(
                      ...
                      children: [
                        const Icon(Icons.videocam, color: kDanger, size: 20),
                        const SizedBox(width: 8),
                        Text(
                          'LIVE DISPATCH FEED (MOCK)',
                          style: GoogleFonts.rajdhani(
                              color: Colors.white,
                              fontSize: 12,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        // Advanced AI Insight Corridor (Phase 4)
        if (result.recommendedLane != null || result.reasoning != null)
          Container(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    const Icon(Icons.auto_awesome, color: kAiCyan, size: 16),
                    const SizedBox(width: 8),
                    Text(
                ...
                if (result.recommendedLane != null) ...[
                  const SizedBox(height: 12),
                  Row(
                    children: [
                      Container(
